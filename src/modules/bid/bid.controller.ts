...
import { IsString, IsOptional, IsObject, IsArray, Min, Max } from 'class-validator';
import { Transform } from 'class-transformer';
import { BidService, CreateBidDto as ICreateBidDto, UpdateBidDto as IUpdateBidDto } from './bid.service';
...
export class BidController {
  ...
  @ApiResponse({ status: 400, description: 'Tender closed or bid already exists' })
  ...
  @ApiOperation({
    ...
    description: 'Admin sees all, Users see bids for their tenders, Vendors see their own bids',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({ status: 400, description: 'Cannot update submitted bid or tender closed' })
  @ApiResponse({ status: 403, description: 'Access denied' })
  @ApiResponse({ status: 404, description: 'Bid not found' })
  async updateBid(
    @Param('id') id: string,
  ...
  @ApiOperation({
    summary: 'Submit bid',
    description: 'Submit draft bid for evaluation. Cannot be modified after submission.',
  })
  @ApiResponse({
    status: 200,
    description: 'Bid submitted successfully',
  })
  @ApiResponse({ status: 400, description: 'Bid already submitted or tender closed' })
  @ApiResponse({ status: 403, description: 'Access denied' })
  @ApiResponse({ status: 404, description: 'Bid not found' })
