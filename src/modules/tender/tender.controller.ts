...
import { TenderService, CreateTenderDto as ICreateTenderDto, UpdateTenderDto as IUpdateTenderDto } from './tender.service';
import { Roles } from '../../common/decorators/roles.decorator';
import { CheckAbilities, AbilityRequirement } from '../../common/decorators/ability.decorator';
...
export class TenderController {
  ...
  @ApiOperation({
    ...
    description: 'Admin creates tenders with global scope, Users create department-scoped tenders',
  })
  @ApiResponse({
    status: 201,
  ...
  async createTender(@Body() createTenderDto: CreateTenderDto, @Req() req: Request) {
  ...
  @ApiOperation({
    ...
    description: 'Admin sees all, Users see department/global, Vendors see published only',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Get detailed tender information with role-based data filtering',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Update tender details. Restrictions apply for published tenders with bids',
  })
  @ApiResponse({
    status: 200,
    description: 'Tender updated successfully',
  })
  @ApiResponse({ status: 400, description: 'Cannot update tender with existing bids' })
  @ApiResponse({ status: 403, description: 'Insufficient permissions' })
  @ApiResponse({ status: 404, description: 'Tender not found' })
  async updateTender(
    @Param('id') id: string,
  ...
  @ApiOperation({
    ...
    description: 'Publish draft tender to make it available for vendors to submit bids',
  })
  @ApiResponse({
    status: 200,
    description: 'Tender published successfully',
  })
  @ApiResponse({ status: 400, description: 'Only draft tenders can be published' })
  @ApiResponse({ status: 403, description: 'Insufficient permissions' })
  @ApiResponse({ status: 404, description: 'Tender not found' })
  ...
  @ApiOperation({
    ...
    description: 'Delete draft tender. Only admins can delete published tenders',
  })
  @ApiResponse({
    status: 200,
    description: 'Tender deleted successfully',
  })
  @ApiResponse({ status: 400, description: 'Only draft tenders can be deleted' })
  @ApiResponse({ status: 403, description: 'Insufficient permissions' })
  @ApiResponse({ status: 404, description: 'Tender not found' })
